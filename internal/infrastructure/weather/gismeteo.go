...
// Supported cities are defined in a hand-curated map (gismeteoCities) keyed by the
// Open-Meteo geocoding id (== LocationKey(geo), the location_id stored on every
// weather_user_cities row). Cities outside the map fall back to Open-Meteo only;
// callers check Supports before calling Forecast/ForecastBatch.
//
...
// caller-supplied location_id is absent from the curated gismeteoCities map. The
// caller should fall back to Open-Meteo-only data — gismeteo coverage is limited by
// design and does not grow automatically with new user subscriptions.
...
// gismeteoCities maps a canonical location_id (the Open-Meteo geocoding id as a
// decimal string, == LocationKey(geo)) to its gismeteo URL components.
//
// How to extend: (1) look up the city's Open-Meteo id via
// https://geocoding-api.open-meteo.com/v1/search?name=<city>&count=1 → results[0].id;
// (2) find the gismeteo.kz URL for the city (www.gismeteo.kz/catalog/…) and note the
// slug and numeric id in the path weather-<slug>-<id>/; (3) add an entry here and add
// a test case to TestGismeteo_curatedMap.
var gismeteoCities = map[string]gismeteoCity{
	// 1526384 = Almaty, Kazakhstan (Open-Meteo geocoding id)
	"1526384": {slug: "almaty", id: 5205},
	// 1526273 = Astana, Kazakhstan
	"1526273": {slug: "astana", id: 5164},
	// 1518980 = Shymkent, Kazakhstan
	"1518980": {slug: "shymkent", id: 5324},
	// 524901 = Moscow, Russia
	"524901": {slug: "moscow", id: 4368},
}
...
// curated map via plain HTTP. Construct with NewGismeteo; do not copy after first use.
...
// proxyURL when non-empty (direct connection otherwise).
//
...
func NewGismeteo(proxyURL string) (*Gismeteo, error) {
...
// Use in tests when you need a fixed production base URL; prefer newGismeteoForTest
// when you also need to redirect requests to an httptest.Server.
func newGismeteoWithClient(client *http.Client) *Gismeteo {
	return &Gismeteo{httpClient: client, baseURL: gismeteoBaseURL}
}

// newGismeteoForTest creates a Gismeteo provider with an injected HTTP client and
// base URL. Use in tests to redirect requests to an httptest.Server without live
// network access.
func newGismeteoForTest(client *http.Client, baseURL string) *Gismeteo {
	return &Gismeteo{httpClient: client, baseURL: baseURL}
}

// Supports reports whether locationID has an entry in the curated city map and
// gismeteo data is therefore obtainable for that location.
func (g *Gismeteo) Supports(locationID string) bool {
	_, ok := gismeteoCities[locationID]
	return ok
}
...
// base URL and curated city entry: <base>/weather-<slug>-<id>/.
func buildGismeteoURL(baseURL string, c gismeteoCity) string {
	return fmt.Sprintf("%s/weather-%s-%d/", baseURL, c.slug, c.id)
}
...
func (g *Gismeteo) get(ctx context.Context, rawURL string) ([]byte, error) {
	...
	req.Header.Set("User-Agent", gismeteoUserAgent)
