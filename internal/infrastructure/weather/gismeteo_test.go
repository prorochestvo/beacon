...
// testGismeteoCities is the coverage fixture injected into test providers. It mirrors
// the four cities seeded into weather_gismeteo_cities; the canonical seed-correctness
// assertion lives in the repository test (TestWeatherGismeteoCityRepository_ObtainGismeteoCoverage).
var testGismeteoCities = map[string]domain.WeatherGismeteoCity{
...
func TestNewGismeteo(t *testing.T) {
	t.Parallel()

	t.Run("injects coverage, base URL, and User-Agent", func(t *testing.T) {
		t.Parallel()
		g, err := NewGismeteo("", testGismeteoCities, "https://example.test", "Custom-UA/1.0")
		require.NoError(t, err)
		require.NotNil(t, g)
		assert.Equal(t, "https://example.test", g.baseURL)
		assert.Equal(t, "Custom-UA/1.0", g.userAgent)
		assert.True(t, g.Supports("1526384"), "injected coverage must drive Supports")
		assert.False(t, g.Supports("9999999"))
	})

	t.Run("empty base URL and User-Agent fall back to compiled-in constants", func(t *testing.T) {
		t.Parallel()
		// A partially-populated weather_sources row (empty base_url / user_agent) must
		// still produce a working provider using the compiled-in defaults.
		g, err := NewGismeteo("", testGismeteoCities, "", "")
		require.NoError(t, err)
		assert.Equal(t, gismeteoBaseURL, g.baseURL)
		assert.Equal(t, gismeteoUserAgent, g.userAgent)
	})

	t.Run("trailing slash in base URL is trimmed", func(t *testing.T) {
		t.Parallel()
		// A base_url written with a trailing slash must not produce a double slash in
		// buildGismeteoURL (which would 404).
		g, err := NewGismeteo("", testGismeteoCities, "https://example.test/", "")
		require.NoError(t, err)
		assert.Equal(t, "https://example.test", g.baseURL)
	})

	t.Run("nil coverage map yields a provider that supports nothing", func(t *testing.T) {
		t.Parallel()
		g, err := NewGismeteo("", nil, "", "")
		require.NoError(t, err)
		require.NotNil(t, g)
		assert.False(t, g.Supports("1526384"), "nil coverage means gismeteo is disabled, not a panic")
	})

	t.Run("invalid proxy URL returns an error", func(t *testing.T) {
		t.Parallel()
		_, err := NewGismeteo("http://\x7f", testGismeteoCities, "", "")
		require.Error(t, err)
...
func TestBuildGismeteoURL(t *testing.T) {
	...
	t.Run("escapes special characters in the slug", func(t *testing.T) {
		t.Parallel()
		got := buildGismeteoURL(gismeteoBaseURL, gismeteoCity{slug: "san jose", id: 123})
		assert.Equal(t, "https://www.gismeteo.kz/weather-san%20jose-123/", got)
	})
}
...
func TestGismeteo_Supports(t *testing.T) {
	...
	g := newGismeteoWithClient(http.DefaultClient, testGismeteoCities)
...
func TestGismeteo_Forecast(t *testing.T) {
	...
	t.Run("returns observation for supported location", func(t *testing.T) {
		...
		defer srv.Close()
		g := newGismeteoForTest(srv.Client(), srv.URL, testGismeteoCities)
