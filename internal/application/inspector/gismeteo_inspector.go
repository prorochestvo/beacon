...
// NewGismeteoInspector returns an advisory Inspector for the gismeteo.kz host using
// the compiled-in default probe URL.
func NewGismeteoInspector() *GismeteoInspector {
	return NewGismeteoInspectorWithURL("")
}

// NewGismeteoInspectorWithURL returns an advisory Inspector that probes baseURL for
// reachability. An empty baseURL falls back to the compiled-in gismeteoProbeURL, so a
// blank weather_sources.base_url keeps the default. Use it to align the health probe
// with the base_url configured in the gismeteo weather_sources row.
func NewGismeteoInspectorWithURL(baseURL string) *GismeteoInspector {
	if baseURL == "" {
		baseURL = gismeteoProbeURL
	}
