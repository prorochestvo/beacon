...
func (s SourcesState) Visible() []api.SourceResponse {
	out := make([]api.SourceResponse, 0, len(s.All))
...
func NewSourcesPage(sources []api.SourceResponse, stats api.StatsResponse, client *apiclient.Client) *SourcesPage {
	return &SourcesPage{
		state: SourcesState{
			All:          sources,
			Stats:        stats,
