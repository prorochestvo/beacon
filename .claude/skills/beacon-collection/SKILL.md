...
description: How Beacon's collector reaches upstreams and what it does with the results — the source kind vocabulary (BID/ASK/LAST) and the options JSON column that carries per-source fetch behaviour, per-source proxy opt-in and why direct is the default, why Telegram Bot API traffic bypasses any proxy unconditionally and why the standard HTTPS_PROXY/HTTP_PROXY/NO_PROXY variables do nothing here, batched sources sharing one fetch (the 20 Yahoo rows), the Open-Meteo weather provider with its retry policy and alert edge semantics, the 16-day long-range forecast on its own daily gate and the content-gated outlook digest, the source-health alerting that reports a source gone silent, and cmd/doctor, the operator-only umbrella for LLM rule generation and source auditing. Load before touching cmd/collector, internal/tools/rateextractor, internal/tools/proxyutil, internal/application/collection, internal/infrastructure/weather, internal/infrastructure/telegrambot/tbotclient.go, notification.SourceHealthAgent, any rate_sources row or seed migration, a source kind or domain.RateSourceOptions, anything involving BEACON_PROXY_URL, options.use_proxy, BEACON_CHROMIUM_PATH or fetcher_kind='chromedp' sources, weather alert kinds, the rain/thaw/heat/frost latches, collection.WeatherForecastAgent, OpenMeteo.Forecast or ForecastRange, the forecast_outlook notify kind and its notify_state signature, or running cmd/doctor rulegen or audit.
---
